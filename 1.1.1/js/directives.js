//Directive to set the width of wrapper elements
angularApp.directive("setwidth", function ($window) {
    ...
    return function (scope, element) {
        ...
        onResizeFunction = function () {
            ...
            var newwidth = Math.floor(document.documentElement.clientWidth / 250) * 250;
        ...
        //Run the function whenever the window changes size.  
...
//Directive to scroll lock an element on the y axis
angularApp.directive("ylock", function ($window) {
...
//Directive to check if an email address is taken
angularApp.directive("emailcheck", function (Validate, Format, $timeout) {
    ...
    return {
        ...
        link: function (scope, element, attrs, ctrl) {
            ...
            //Everytime the Input is blurred...
            element.on('change', function () {

                //Check to see if the display name is open  
                Validate.emailOpen(Format.removespaces(registerForm.email.value))
                ...
                .then(function (data) {
                    ...
                    if (data) {
                        ...
                        $timeout(function () {
                            ctrl.$setValidity('emailtaken', false);
                        }, 0);

                    } else {

                        $timeout(function () {
                            ctrl.$setValidity('emailtaken', true);
                        }, 0);
...
//Directive to check if a Display Name is open
angularApp.directive("displaynamecheck", function (Validate, Format, $timeout) {
    ...
    return {
        ...
        link: function (scope, element, attrs, ctrl) {
            ...
            element.on('change', function () {
                ...
                //Check to see if the display name is open  
                Validate.displaynameOpen(Format.removespaces(registerForm.displayname.value))

                .then(function (data) {

                    //If the Display Name is Taken...
                    if (data) {

                        $timeout(function () {
                            ctrl.$setValidity('displaynametaken', false);
                        }, 0);

                    } else {

                        $timeout(function () {
                            ctrl.$setValidity('displaynametaken', true);
                        }, 0);
...
//Directive to load an image from file
angularApp.directive('loadimage', function (Image) {
    return {
        restrict: 'A',
        link: function (scope, element, attrs) {

            //If the forms "file input box" changes, run the following 
            element.bind('change', function () {

                //Make a FileReader object called "reader"
                var reader = new FileReader();

                //This "onload" handler fires the event, whenever the file reader completes a read.
                reader.onload = function (event) {
                    ...
                    tempimg.onload = function () {
                        ...
                        //Validate the Image to ensure it meets the minimum requirements.
                        Image.validate(tempimg)

                        //Validation succeeded
                        .then(function (result) {
                                                                                 
                            //update the form model in the controller scope that the image has changed
                            scope.form.imagechanged = true;

                            //Create an object reference to the Visible Onscreen Image Tag. 
                            var onscreenimg = document.getElementById(element[0].nextElementSibling.id);

                            //Set the onscreen image src to the src of the temp image
                            onscreenimg.src = tempimg.src
                            
                            //Call Orientation Factory
                            Image.orientation(onscreenimg)
                            ...
                            //Use this if commenting out orientation factory
                            //tempimg.remove();
                        ...
                        .catch(function (error) {
                            ...
                            //Set the onscreen image src to the src of the error image
                            onscreenimg.src = "/1.1.1/img/interface/errors/" + error + ".png"

                            //scope.form.mode = error;
                            console.log(error);
                ...
                //Use the Reader to read the file from the file input element
                reader.readAsDataURL(document.getElementById(element[0].id).files[0]);
...
//Directive to load an image from drop
angularApp.directive('dropimage', function() {
    return {
      restrict: 'A',
      scope: {
        ctrlFn : '&',
      ...
      link: function(scope, element, attrs) {
        ...
        validMimeTypes = attrs.dropimage;
            ...
            if (checkSize(size) && isTypeValid(type)) {
              return scope.$apply(function() {
                ...
                //scope.form.imagechanged = true;
                  ...
                  //scope.ctrlFn({arg1: 'neener neener'});
                  scope.ctrlFn();
...
//Directive for Fullscreen Slider Resize 
angularApp.directive('resize',['$window', function ($window) {
    return {
        scope:true,
        link:    function (scope, element,attrs) {
...
//Directive for Fullscreen Slider
angularApp.directive('slider',['$window','$location',
    function ($window,$location) {
    return {
        priority: 0,
        restrict: 'A,C',
        scope: false,
        templateUrl:'/1.1.1/html/components/slider.html',
        link: function (scope, element, attrs) {
...
//Custom Filter for Unique in ngRepeat
